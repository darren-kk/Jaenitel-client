...
첫번째 기능인 명령어 입력은 input 창에서 Enter 키 입력 이벤트가 발생했을때, 해당 명령어에 대응하는 조건문을 통해 구현이 가능 했습니다.
...
두번째 기능인 스크롤 조절 기능은 `React의 ref`를 활용하여 구현이 가능했습니다.
...
`React` 자체에서는 이러한 작업을 직접적으로 수행할 수 있는 내장 방법을 제공하지 않기 때문에, `DOM` 노드에 대한 직접적인 참조가 필요합니다. 이를 위해 저는 **React의 ref**를 활용 할 수 있었습니다.
...
그러나 저의 어플리케이션은 독특하게 이와 반대되는 부분을 고민했습니다. 좋지 않았던 통신환경을 재현하기 위해, 서버에서 데이터 전송에 인위적인 지연(재밍)을 주는 방식과 클라이언트 측에서 의도적으로 느린 렌더링을 구현하는 두 가지 방법을 고려했습니다.
...
- **유연한 명령어 입력 처리**: 사용자가 명령어를 빠르게 입력하는 과정에서 생길 수 있는 오류를 줄이기 위해, 명령어 입력의 유연성을 높였습니다. 예를 들어, 게시글 이동을 위해 "12 go"와 같은 명령어를 입력할 때 띄어쓰기 오류를 최소화하기 위해 입력 로직을 개선했습니다. 이러한 세심한 조정은 사용자가 더욱 원활하게 어플리케이션을 사용할 수 있도록 합니다.
...
이러한 요소들은 사용자가 불편함을 경험하면서도 어플리케이션의 독특한 컨셉을 즐길 수 있도록 설계되었습니다. 사용자의 행동을 예측하고, 그들의 경험을 향상시키기 위한 지속적인 고민과 노력이 녹아 들도록 노력했습니다. 이를 통해, 사용자들은 단순한 불편함 너머, 의도된 불편함을 통한 재미를 느끼길 기대했습니다.
...
이 변경은 실시간 통신의 빠른 반응성을 유지하면서도 DB 저장의 필요성을 충족시키는 방안으로, 성능과 효율성을 크게 향상시켰습니다. 프로젝트에서 성능 최적화를 위해 신중하게 로직을 검토하고, 실험을 통해 데이터 기반 결정을 내림으로, 가정 기반의 로직 변경이 보다 구체적이고 근거있는 방향성으로 개선될 수 있었습니다.
...
보안과 관련된 기능을 구현할 때는 최신 웹 표준을 준수하는 것이 중요하며, 이를 위해 사용하는 프레임워크와 라이브러리의 최신 버전을 유지하는 것이 필수임을 배울 수 있는 값진 경험이었습니다.
