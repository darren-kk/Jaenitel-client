...
- **polygon**: `polygon` 함수는 다각형의 모양을 정의하여 `clip-path`에서 사용합니다. 이 함수는 다각형의 각 꼭짓점의 좌표를 백분율로 나타냅니다.
...
이 프로젝트를 통해, 사용자들에게 과거의 기술적 불편함을 체험시키면서 현대적인 기법을 통해 구현된 새로운 방식의 `ineratction`을 제공하고자 했습니다.
...
- **안내 페이지로의 유도**: 초기 사용자들의 피드백을 바탕으로, 사용자가 어플리케이션의 컨셉을 쉽게 이해할 수 있도록 최초 홈페이지 진입 시 안내 페이지를 통해 전반적인 사용 흐름을 설명하는 방식을 채택했습니다. 이를 통해 사용자가 불편함 속에서도 어플리케이션의 목적과 사용법을 자연스럽게 파악할 수 있도록 했습니다.
  <br>
...
- **자동 Focusing과 onClick Event**: 마우스 대신 키보드 중심의 조작 방식을 채택함으로써, 사용자가 명령어 입력에 더 자연스럽게 접근할 수 있도록 했습니다. 페이지 로딩 시 자동으로 명령어 입력 창에 포커스가 맞춰지며, 화면 어디를 클릭해도 다시 명령어 입력 창으로 돌아오는 방식을 구현했습니다. 이로써, 사용자가 어플리케이션의 조작법에 쉽게 적응 할 수 있게끔 유도하였습니다.
...
이러한 요소들을 통해 사용자가 불편함을 경험하면서도 어플리케이션의 독특한 컨셉을 즐길 수 있도록 설계하였습니다. 사용자의 행동을 예측하고, 사용 경험을 향상시키기 위한 지속적인 고민과 노력이 녹아 들도록 노력했습니다. 궁극적으로 사용자들이 단순한 불편함 너머, 의도된 불편함을 통한 재미를 느끼길 기대했습니다.

### 2-2. 모두 다른 화면크기, 혹시 내 프로젝트 UI가 깨지진 않을까?

반응형 디자인과 데이터 최적화

현대 웹 어플리케이션에서 반응형 디자인은 필수적입니다. 사용자들은 다양한 화면 크기와 해상도를 가진 장치를 사용하므로, 이에 적절히 대응하는 UI 설계가 매우 중요하다고 생각합니다. 이를 위해, 클라이언트의 뷰포트(Viewport) 크기에 따라 게시글의 수를 조절하고, 서버에서 이에 맞추어 데이터를 제공하는 로직을 구현했습니다.
...
위 로직은 클라이언트의 화면 크기 변화에 따라 적절한 수의 게시글을 보여주는 로직입니다.. 각 게시글의 높이를 고려하여, 사용 가능한 공간에 맞게 페이지당 표시할 게시글 수를 동적으로 조절합니다.
...
그러나 이 방식은 소켓 이벤트의 빠른 처리와 DB 저장의 복잡성이 뒤섞이며, 성능 저하의 가능성이 있을 수 있다고 판단했습니다.. 실시간 응답을 요구하는 소켓 통신의 특성상, DB 저장 로직이 소켓 이벤트 처리 속도에 영향을 미칠 수 있었습니다.
...
이 변경으로 실시간 통신의 빠른 반응성을 유지하면서도 DB 저장 역시 별도로 가능케 하여, 성능과 효율성을 크게 향상시켰습니다.
